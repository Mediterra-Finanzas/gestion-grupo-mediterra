...
// Extrae pares (vista, columna_orden) de cada procSelect("proc_v_*_listado", `...order=col...`)
const re = /procSelect\(\s*"(proc_v_[a-z_]+_listado)"\s*,\s*`([^`]*)`/g;
let m, encontrados = 0;
while ((m = re.exec(src)) !== null) {
  ...
  ok(!!cols, `${vista}: falta manifiesto de columnas (agregar a COLS)`);
  if (cols) ok(setOf(cols).has(col), `${vista} ordena por columna inexistente: "${col}"`);
}

ok(encontrados >= 6, `debía encontrar >=6 listados con order= (encontró ${encontrados})`);
