-- ============================================================
-- CURRENCY DOMAIN FASE 1 — EJECUCIÓN TRANSACCIONAL v3
-- OA-013-07 — CANDIDATE FOR EXECUTION (FINAL)
-- Supersede a: 002_currency_transactional.sql   (NEVER EXECUTED)
--              002_currency_transactional_v2.sql (NEVER EXECUTED)
-- ============================================================
-- OA-013-06 MATRIZ DE CONSTRAINTS (PASO1-DES-001 v2.1 vs este SQL):
--
-- Regla aprobada                                 | v2           | v3              | Estado
-- ----------------------------------------------|-------------|-----------------|-------
-- moneda_origen <> moneda_destino                | chk_monedas | chk_monedas     | PASS
-- valor > 0                                      | chk_valor_pos| chk_valor_pos  | PASS
-- estado IN (activo/supersedido/inv.)            | chk_estado  | chk_estado      | PASS
-- es_manual → ingresado_por/en/motivo NN        | PARCIAL(*)  | chk_manual v3   | PASS
-- es_manual → estado_aprobacion <> 'auto'       | FALTABA     | chk_manual v3   | PASS
-- estado_aprobacion domain                       | chk_aprobacion| chk_aprobacion| PASS
-- aprobado/rechazado → aprobado_por/en NN       | FALTABA     | chk_apr_evidencia| PASS
-- avg types → methodology_metadata NN            | chk_metadata| chk_metadata    | PASS
-- unicidad activos (par+fecha+type+purpose)      | INCOMPLETO(**)| uq_activos v3 | PASS
...
--
-- (*) v2: solo validaba ingresado_por, ignoraba ingresado_en, motivo y 'auto'.
-- (**) v2: uq_currency_tc_activo solo usaba (par+fecha) — insuficiente per OA-013-01.
--      La identidad lógica correcta es: par + fecha + rate_type + rate_purpose.
-- ============================================================
-- REVISIÓN OA-011-01 / OA-013 (válida para v3):
--   DELETE: NINGUNO
--   DROP: NINGUNO
...
-- ─────────────────────────────────────────────────────────────
-- SECCIÓN A: DDL CON CONSTRAINTS COMPLETOS (OA-012-04 / OA-013-02/03)
-- ─────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS currency_tc (
  ...
  -- OA-013-02: Fortalecido — registro manual requiere los 4 campos y no puede usar 'auto'.
  -- v2 solo validaba ingresado_por; aquí se valida ingresado_en, motivo y estado_aprobacion.
  CONSTRAINT chk_manual
    CHECK (
      NOT es_manual
      OR (
        ingresado_por       IS NOT NULL
        AND ingresado_en    IS NOT NULL
        AND motivo          IS NOT NULL
        AND estado_aprobacion IN ('pendiente', 'aprobado', 'rechazado')
      )
    ),
  ...
  -- OA-013-03: Nueva — evidencia obligatoria al aprobar o rechazar.
  CONSTRAINT chk_aprobacion_evidencia
    CHECK (
      estado_aprobacion NOT IN ('aprobado', 'rechazado')
...
-- OA-013-01: Identidad lógica correcta para unicidad de activos.
-- v2 usaba solo (par+fecha) — insuficiente: dos registros activos para el mismo par+fecha
-- pero diferente rate_type o rate_purpose son entidades distintas y deben coexistir.
-- La identidad lógica completa es: par + fecha + rate_type + rate_purpose.
CREATE UNIQUE INDEX IF NOT EXISTS uq_currency_tc_activo
  ON currency_tc (moneda_origen, moneda_destino, fecha, rate_type, rate_purpose)
  WHERE estado = 'activo';
...
VALUES (
  ...
  'in_progress',
  ARRAY['maestro_tc'],
  'Fase 1 v3 — migración de 48 registros OBSERVADOS en maestro_tc. USD-CLP: 24, EUR-CLP: 24. OA-010-04 / OA-011-04 / OA-012-07 / OA-013-07.'
);
...
ORDER BY tablename;
-- Esperado: 5 filas, rowsecurity = true en todas
...
--   002_currency_transactional.sql    → SUPERSEDED / NEVER EXECUTED
--   002_currency_transactional_v2.sql → SUPERSEDED / NEVER EXECUTED
--   002_currency_transactional_v3.sql → CANDIDATE FOR EXECUTION (OA-013-07)
-- ============================================================
