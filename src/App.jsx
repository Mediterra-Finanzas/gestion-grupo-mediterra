...
// Banner flotante de "nueva versión" — se inyecta DIRECTO al DOM para que sea
// visible SOBRE CUALQUIER pantalla (el hub o dentro de un módulo como Rendiciones).
// Antes el banner solo existía en la vista del hub, así que quien trabajaba dentro
// de un módulo nunca lo veía y se quedaba en el código viejo. No recarga solo: el
// usuario elige el momento para no perder lo que está cargando.
let _bannerVersionDescartado = false;
function mostrarBannerNuevaVersion() {
  if (_bannerVersionDescartado) return;
  if (typeof document === "undefined") return;
  if (document.getElementById("mediterra-banner-version")) return;
  const wrap = document.createElement("div");
  wrap.id = "mediterra-banner-version";
  wrap.style.cssText = "position:fixed;bottom:20px;right:20px;z-index:2147483647;max-width:340px;background:#ffffff;color:#0f172a;padding:14px 18px;border-radius:12px;box-shadow:0 8px 32px rgba(0,0,0,.28);border:1px solid #e2e8f0;font-family:sans-serif;font-size:13px";
  const t1 = document.createElement("div");
  t1.style.cssText = "font-weight:700;margin-bottom:6px";
  t1.textContent = "🔄 Nueva versión disponible";
  const t2 = document.createElement("div");
  t2.style.cssText = "font-size:12px;color:#64748b;line-height:1.5;margin-bottom:10px";
  t2.textContent = "Puedes seguir trabajando sin problema. Cuando termines lo que estás cargando, haz clic en Actualizar para usar la versión nueva.";
  const row = document.createElement("div");
  row.style.cssText = "display:flex;gap:8px;justify-content:flex-end";
  const bDespues = document.createElement("button");
  bDespues.textContent = "Después";
  bDespues.style.cssText = "background:none;border:1px solid #e2e8f0;color:#64748b;border-radius:8px;padding:6px 14px;cursor:pointer;font-weight:600;font-size:12px";
  bDespues.onclick = () => { _bannerVersionDescartado = true; wrap.remove(); };
  const bActualizar = document.createElement("button");
  bActualizar.textContent = "Actualizar";
  bActualizar.style.cssText = "padding:6px 16px;border-radius:8px;background:#0f766e;color:#fff;border:none;cursor:pointer;font-weight:700;font-size:12px";
  bActualizar.onclick = () => window.location.reload();
  row.appendChild(bDespues); row.appendChild(bActualizar);
  wrap.appendChild(t1); wrap.appendChild(t2); wrap.appendChild(row);
...
export default function App(){
  ...
  // (Checker viejo por ETag/x-vercel-id desactivado: poco confiable — daba falsos
  //  positivos. El checker por hash de bundle de más abajo es la fuente única.)
      ...
      try {
        ...
        if(bundle !== currentBundle) {
          ...
          if(document.hidden) {
            ...
            // Está trabajando: banner no intrusivo, visible en cualquier pantalla.
            // Se recargará solo cuando deje la pestaña (recargarSiOculto).
