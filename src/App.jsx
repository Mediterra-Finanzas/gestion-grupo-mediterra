...
function startVersionChecker(setNuevaVersion) {
  async function check() {
    try {
      const res = await fetch("/?_v="+Date.now(), {method:"HEAD"});
      const etag = res.headers.get("etag") || res.headers.get("x-vercel-id") || "";
      if(_lastBuildId && etag && etag !== _lastBuildId) {
        setNuevaVersion(true);
      }
      if(etag) _lastBuildId = etag;
    } catch(e) { /* offline o error — ignorar */ }
  }
  check();
  setInterval(check, 60000); // cada 60 segundos
}
...
export default function App(){
  ...
  // Iniciar detector de nueva versión
  useEffect(()=>{ startVersionChecker(setNuevaVersion); },[]);
