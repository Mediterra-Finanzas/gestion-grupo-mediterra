...
describe("prototipo · el riel cambia de forma con el ancho", () => {
  ...
  test("al ensanchar la ventana la tabla vuelve, sin recargar", () => {
    ...
    // Candidato diseno + alertas: sin la bandeja de cobranza queda una sola
    // tabla densa en el home, la de contratos. Se identifica por su `caption`.
    expect(screen.getByRole("table", { name: /Contratos/ })).toBeInTheDocument();
    expect(screen.queryAllByRole("table").length).toBeGreaterThanOrEqual(1);
  });
