-- ============================================================================
-- R4-B_PREFLIGHT_FINAL.sql  — READ ONLY. Gate exacto antes de mutar R4-B.
-- TARGET: Supabase STAGING (nlvfjpwiecgrosjnwwik). Production = HANDS-OFF.
-- No muta nada (solo SELECT). Devuelve una grilla check/expected/actual/status
-- + una fila VERDICT final. Si VERDICT != PASS(exact) => HARD STOP, NO correr la mutacion.
-- Autoritativo: has_function_privilege(...) para EXECUTE efectivo; aclexplode(proacl)
-- para PUBLIC explicito (has_function_privilege no acepta el pseudo-rol PUBLIC).
-- ============================================================================
WITH throttle AS (SELECT ARRAY['proc_fn_auth_attempt','proc_fn_auth_reset']::text[] AS names),
op AS (  -- funciones operacionales = proc_fn_* menos throttle
...
checks AS (
  SELECT '01_anon_grants_proc'        AS check_name, '278' AS expected,
  ...
  UNION ALL SELECT '07_op_anon_effective_exec','70',
         (SELECT count(*)::text FROM op WHERE has_function_privilege('anon', op.oid, 'EXECUTE'))
  UNION ALL SELECT '08_op_public_exec','70',
...
SELECT 'ZZ_VERDICT', 'PASS', (SELECT count(*) FILTER (WHERE status='FAIL')::text FROM scored)||' fail',
       (SELECT CASE WHEN bool_and(status='PASS') THEN 'PASS(exact)' ELSE 'FAIL-HARDSTOP' END FROM scored)
ORDER BY 1;
