...
-- NO crea/altera/borra nada. Si aborta → HARD STOP (no correr R3-S1..S5).
...
-- cohorte); (c) estado PRE-R3 (binding/throttle/helpers-v2/whoami AÚN ausentes).
-- ============================================================================
SELECT
  ...
  (SELECT count(*) FROM iam_usuario_empresa
     WHERE activo AND empresa_id='5aa10886-2a76-4a9e-9bc3-303fb776cd49')          AS als_memberships,        -- 6 (post-R2)
  -- estado PRE-R3 (deben estar TODOS ausentes):
...
DO $s0$
DECLARE
  v_als int; v_proc int; v_bgrant int; v_main int; v_iam boolean; v_mem int;
  v_bind boolean; v_thr boolean; v_v2 boolean;
BEGIN
  ...
  IF v_als<>1 THEN RAISE EXCEPTION 'R3-S0 ABORT: ALS no exacto (%). Target/Core. HARD STOP.', v_als; END IF;
  IF v_proc<30 THEN RAISE EXCEPTION 'R3-S0 ABORT: proc_* = % (<30). Target NO staging. HARD STOP.', v_proc; END IF;
  IF v_bgrant<1 THEN RAISE EXCEPTION 'R3-S0 ABORT: bridge DEV_ONLY ausente. Target sospechoso. HARD STOP.'; END IF;
  IF v_main<>1 THEN RAISE EXCEPTION 'R3-S0 ABORT: calendario_data.main ausente. HARD STOP.'; END IF;
  IF NOT v_iam THEN RAISE EXCEPTION 'R3-S0 ABORT: iam_* ausente. Correr R1/R2 primero. HARD STOP.'; END IF;
  IF v_mem<>6 THEN RAISE EXCEPTION 'R3-S0 ABORT: memberships ALS = % (esperado 6, post-R2). HARD STOP.', v_mem; END IF;
  IF v_bind OR v_thr OR v_v2 THEN
    RAISE EXCEPTION 'R3-S0 ABORT: estado NO es PRE-R3 (binding=%, throttle=%, helpers_v2=%). R3 ya parcialmente aplicado. STOP y revisar.', v_bind, v_thr, v_v2;
  END IF;

  RAISE NOTICE 'R3 PRE-STATE OK: staging (proc_*=%, anon_grants=%), post-R2 (iam presente, 6 memberships ALS), pre-R3 (binding/throttle/helpers-v2 ausentes). Listo para R3-S1 (esperar autorización).', v_proc, v_bgrant;
END
$s0$;
