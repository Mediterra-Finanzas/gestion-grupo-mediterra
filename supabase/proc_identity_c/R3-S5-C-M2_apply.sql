-- ============================================================================
-- R3-S5-C-M2_apply.sql — Fixture multi-membership/cross-tenant (OPCIÓN B).
-- Crea SOLO objetos fixture marcados R3S5 (2 empresas, 1 iam sintético, 2 memberships,
-- 2 filas proc_repaletizaje). NO toca calendario_data/main/pins, ni Angelo/Carol/Osiris/ALS.
-- PRECONDICIÓN: el usuario auth.users 'uat-multi-r3s5@fixture.invalid' ya fue creado en el
-- Dashboard (Authentication → Users → Add user, Auto Confirm). El binding se resuelve por email.
-- Relacional, acotado, idempotente (aborta si ya hay fixture), affected_rows exactos, reversible.
-- TARGET: staging nlvfjpwiecgrosjnwwik. Producción = HANDS-OFF.
--
-- Identificadores fixture (uuids fijos):
--   empresa A = f1000000-0000-0000-0000-0000000000aa  (codigo UAT-A-R3S5)
--   empresa B = f1000000-0000-0000-0000-0000000000bb  (codigo UAT-B-R3S5)
--   iam user  = f1000000-0000-0000-0000-000000000011  (email uat-multi-r3s5@fixture.invalid)
-- ROLLBACK: ver R3-S5-C-M3_rollback.sql
-- ============================================================================
BEGIN;
...
BEGIN
  ...
  IF v_proc<30 THEN RAISE EXCEPTION 'M2 ABORT: proc_*=% (<30). No staging.', v_proc; END IF;
  IF NOT v_iam THEN RAISE EXCEPTION 'M2 ABORT: iam_* ausente.'; END IF;
  IF v_bridge<1 THEN RAISE EXCEPTION 'M2 ABORT: bridge DEV_ONLY ausente.'; END IF;
  IF v_als<>1 THEN RAISE EXCEPTION 'M2 ABORT: ALS no exacto (%).', v_als; END IF;
  IF v_authu<>1 THEN RAISE EXCEPTION 'M2 ABORT: auth.users uat-multi cnt=% (esperado 1). Crea el usuario en el Dashboard primero.', v_authu; END IF;
  IF v_resid_e<>0 OR v_resid_i<>0 THEN RAISE EXCEPTION 'M2 ABORT: fixture residual (empresas=%, iam=%). Correr rollback primero.', v_resid_e, v_resid_i; END IF;
  IF v_als_mem<>6 THEN RAISE EXCEPTION 'M2 ABORT: ALS memberships baseline=% (esperado 6).', v_als_mem; END IF;
  IF v_carol<>0 THEN RAISE EXCEPTION 'M2 ABORT: Carol baseline=% (esperado 0).', v_carol; END IF;
  IF v_ang IS DISTINCT FROM '29b0217d-40ed-4fde-84c0-51d51b98c849'::uuid THEN RAISE EXCEPTION 'M2 ABORT: Angelo binding baseline alterado (%).', v_ang; END IF;
  RAISE NOTICE 'M2 preflight OK: auth uat=1, sin residual, baseline ALS=6/Carol=0/Angelo intacto. Creando fixture…';
END
$pre$;
...
-- Usuario IAM sintético, binding resuelto por email (server-side)
...
BEGIN
  ...
  IF v_e<>2 THEN RAISE EXCEPTION 'M2 POST FAIL: empresas fixture=% (2).', v_e; END IF;
  IF v_u<>1 THEN RAISE EXCEPTION 'M2 POST FAIL: iam fixture=% (1).', v_u; END IF;
  IF v_bind IS DISTINCT FROM v_authid THEN RAISE EXCEPTION 'M2 POST FAIL: binding=% != auth uat=%.', v_bind, v_authid; END IF;
  IF v_m<>2 THEN RAISE EXCEPTION 'M2 POST FAIL: memberships=% (2).', v_m; END IF;
  IF v_r<>2 THEN RAISE EXCEPTION 'M2 POST FAIL: filas proc=% (2).', v_r; END IF;
  IF v_als<>6 THEN RAISE EXCEPTION 'M2 POST FAIL: ALS memberships=% (6) — invariante roto.', v_als; END IF;
  IF v_carol<>0 THEN RAISE EXCEPTION 'M2 POST FAIL: Carol=% (0) — invariante roto.', v_carol; END IF;
  IF v_ang IS DISTINCT FROM '29b0217d-40ed-4fde-84c0-51d51b98c849'::uuid THEN RAISE EXCEPTION 'M2 POST FAIL: Angelo binding alterado (%).', v_ang; END IF;

  RAISE NOTICE 'M2 OK: fixture creado (2 empresas, iam bound a %, 2 memberships, 2 filas proc); ALS=6/Carol=0/Angelo intactos. Commit.', v_authid;
END
$post$;
